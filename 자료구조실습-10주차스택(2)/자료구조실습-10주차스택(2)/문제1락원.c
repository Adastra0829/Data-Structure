...
int priority(char* input, char* p, char o) {//수식주소, 현재주소, 현재주소의 문자
	if (o == '!') return 6;
	else if ((o == '+' || o == '-')) {
		if (input < p && !isOperand(*(p - 1)))
			return 6;
		else if (p == input)
			return 6;
    }
	...
	for (int i = 0; i < n; i++) {
		...
		while (*p) {//식의 끝까지
			...
			if (isOperand(*p)) {
				...
				//printf("else\n");
				while (s->top != NULL && priority(input, p, *p) <= priority(input, 0, top(s))) {//연산자 우선순위 중요
					if (top(s) == '&') {
						printf("&&");  pop(s);
					}
					else if (top(s) == '|') {
...
단일 연산자 때문에 문제가 생기는것 같은데 어떻게 검사를 해주어야할지 모르겠다..
